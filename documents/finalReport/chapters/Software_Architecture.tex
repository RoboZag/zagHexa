...
\end{figure}
		...
		\begin{figure}[h]		
			\centering
			\includegraphics[width =.8\textwidth]{Walking_flow_chart}
...
OR
\lstinline|$ rosdep install rqt_plot|
...
/adc. The code uses the rosserial_arduino::Adc message to handle the ADC value. We can plot the values using the rqt_plot tool.
Following is the command to plot the three axes values in a single plot:
\lstinline|$ rqt_plot adc/adc0 adc/adc1 adc/adc2|
Next is a screenshot of the plot of the three channels of ADC:
\begin{figure}[H]
	\centering
	\includegraphics[width =.8\textwidth]{figures/ADC}
	\caption{Plot of ADC in rqt_plot}
	\label{Fig:ADC}
\end{figure}